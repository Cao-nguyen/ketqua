...
import { useGrade, calculateTBM } from "../context/GradeContext";
import { Plus, Database, X as XIcon } from "lucide-react";
...
const EmptySlot: React.FC<{ onClick: () => void; label?: string }> = ({
  onClick,
  label,
}) => (
  <button
...
const SubjectRow: React.FC<{ subject: SubjectData }> = ({ subject }) => {
  ...
  const calcHk1 = calculateTBM(
    (subject.regularGrades1 || []).map((g) => g.value),
    subject.midtermGrade1?.value ?? null,
    subject.finalGrade1?.value ?? null,
  );

  const calcHk2 = calculateTBM(
    subject.regularGrades.map((g) => g.value),
    subject.midtermGrade?.value ?? null,
    subject.finalGrade?.value ?? null,
  );
  ...
  const currentRegularG =
    activeSemester === "HK1"
      ? subject.regularGrades1 || []
      : subject.regularGrades;
  const currentMidG =
    activeSemester === "HK1" ? subject.midtermGrade1 : subject.midtermGrade;
  const currentFinalG =
    activeSemester === "HK1" ? subject.finalGrade1 : subject.finalGrade;
  ...
  const tbmColorClass =
    tbmValue > 0
      ? getColor(tbmValue).replace("bg-", "text-").split(" ")[1]
      : "text-gray-400";
  const fullYearColorClass =
    fullYearValue > 0
      ? getColor(fullYearValue).replace("bg-", "text-").split(" ")[1]
      : "text-gray-400";
    ...
    <>
      <tr className="group hover:bg-[#fafafa] transition duration-200">
        <td className="p-4 align-top min-w-[150px]">
          ...
          <div
            className="inline-flex items-center gap-1.5 text-[11px] text-gray-500 bg-gray-50/80 hover:bg-blue-50/50 hover:text-blue-600 px-2 py-1 rounded-md cursor-pointer transition border border-gray-100"
            onClick={() => setBonusModalOpen(true)}
          >
            <Database
              size={12}
              className={totalBonus > 0 ? "text-blue-500" : "text-gray-400"}
            />
        ...
        <td className="p-4 align-top text-center border-l border-gray-50/50">
          <div className="flex justify-center">
            {currentMidG ? (
              ...
              <EmptySlot
                onClick={() => handleOpenAdd(GradeType.MIDTERM)}
                label="Nhập giữa kì"
              />
            )}
          </div>
        ...
        <td className="p-4 align-top text-center border-l border-gray-50/50">
          <div className="flex justify-center">
            {currentFinalG ? (
              ...
              <EmptySlot
                onClick={() => handleOpenAdd(GradeType.FINAL)}
                label="Nhập cuối kì"
              />
            )}
          </div>
        ...
        {activeSemester === "HK1" ? (
          <td className="p-4 align-top text-center border-l border-gray-50/50 bg-blue-50/30">
            ...
            {subject.targetTBM1 !== undefined &&
              subject.targetTBM1 !== null &&
              tbmValue > 0 && (
                <div
                  className={`text-[10px] mt-1 font-semibold ${tbmValue >= subject.targetTBM1 ? "text-emerald-500" : "text-red-500"}`}
                >
                  {tbmValue >= subject.targetTBM1 ? "+" : ""}
                  {(tbmValue - subject.targetTBM1).toFixed(2)}
                </div>
              )}
          </td>
        ) : (
            ...
            <td className="p-4 align-top text-center border-l border-gray-50/50">
              {calcHk1 === 0 ? (
                <input
                  ...
                  placeholder="-"
                  value={
                    subject.semester1Average !== null
                      ? subject.semester1Average
                      : ""
                  }
                  onChange={(e) => {
                ...
                <div>
                  <div
                    className={`font-bold text-base ${calcHk1 > 0 ? getColor(calcHk1).replace("bg-", "text-").split(" ")[1] : "text-gray-400"}`}
                  >
                    {calcHk1.toFixed(2)}
                  </div>
                </div>
              ...
              {subject.targetTBM1 !== undefined &&
                ...
                actualHk1 > 0 && (
                  <div
                    className={`text-[10px] mt-1 font-semibold ${actualHk1 >= subject.targetTBM1 ? "text-emerald-500" : "text-red-500"}`}
                  >
                    {actualHk1 >= subject.targetTBM1 ? "+" : ""}
                    {(actualHk1 - subject.targetTBM1).toFixed(2)}
                  </div>
                )}
            </td>
            <td className="p-4 align-top text-center border-l border-gray-50/50 bg-blue-50/30">
              ...
              {subject.targetTBM2 !== undefined &&
                subject.targetTBM2 !== null &&
                calcHk2 > 0 && (
                  <div
                    className={`text-[10px] mt-1 font-semibold ${calcHk2 >= subject.targetTBM2 ? "text-emerald-500" : "text-red-500"}`}
                  >
                    {calcHk2 >= subject.targetTBM2 ? "+" : ""}
                    {(calcHk2 - subject.targetTBM2).toFixed(2)}
                  </div>
                )}
            </td>
